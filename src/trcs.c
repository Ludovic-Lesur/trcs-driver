...
typedef struct {
    ...
    TRCS_output_current_range_t current_range;
    TRCS_output_current_range_t previous_range;
    TRCS_range_context_t range_ctx[TRCS_OUTPUT_CURRENT_RANGE_LAST];
    TRCS_output_current_range_state_t range_state;
    uint8_t switch_pending;
    int32_t output_current_mv_buffer[TRCS_ADC_SAMPLE_BUFFER_SIZE];
    uint8_t output_current_mv_buffer_idx;
    int32_t output_current_mv;
} TRCS_context_t;

/*** TRCS local global variables ***/

static const uint16_t TRCS_SHUNT_RESISTOR_MOHM[TRCS_OUTPUT_CURRENT_RANGE_LAST] = { 50000, 500, 5 };
...
static TRCS_status_t _TRCS_update_adc_data(void) {
    ...
    // Get raw data in mV.
    status = TRCS_HW_adc_get_output_current((int32_t*) &trcs_ctx.output_current_mv_buffer[trcs_ctx.output_current_mv_buffer_idx]);
    if (status != TRCS_SUCCESS) goto errors;
    // Update average.
    math_status = MATH_average((int32_t*) trcs_ctx.output_current_mv_buffer, TRCS_ADC_SAMPLE_BUFFER_SIZE, &trcs_ctx.output_current_mv);
    MATH_exit_error(TRCS_ERROR_BASE_MATH);
    // Manage index.
    trcs_ctx.output_current_mv_buffer_idx++;
    if (trcs_ctx.output_current_mv_buffer_idx >= TRCS_ADC_SAMPLE_BUFFER_SIZE) {
        trcs_ctx.output_current_mv_buffer_idx = 0;
    }
errors:
    return status;
}
...
/*******************************************************************/
static void _TRCS_reset(TRCS_output_current_range_state_t range_state) {
    // Local variables.
    uint8_t idx = 0;
    // Reset state.
    trcs_ctx.current_range = TRCS_OUTPUT_CURRENT_RANGE_HIGH;
    trcs_ctx.previous_range = TRCS_OUTPUT_CURRENT_RANGE_HIGH;
    ...
    for (idx = 0; idx < TRCS_ADC_SAMPLE_BUFFER_SIZE; idx++) {
        trcs_ctx.output_current_mv_buffer[idx] = 0;
    }
    trcs_ctx.output_current_mv_buffer_idx = 0;
    trcs_ctx.output_current_mv = 0;
}
...
static TRCS_status_t _TRCS_set_off_state(uint8_t high_range_state) {
    ...
    status = TRCS_HW_set_output_current_range_state(TRCS_OUTPUT_CURRENT_RANGE_HIGH, high_range_state);
    if (status != TRCS_SUCCESS) goto errors;
    status = TRCS_HW_set_output_current_range_state(TRCS_OUTPUT_CURRENT_RANGE_MIDDLE, 0);
    if (status != TRCS_SUCCESS) goto errors;
    status = TRCS_HW_set_output_current_range_state(TRCS_OUTPUT_CURRENT_RANGE_LOW, 0);
    if (status != TRCS_SUCCESS) goto errors;
    // Update state.
    _TRCS_reset((high_range_state == 0) ? TRCS_OUTPUT_CURRENT_RANGE_STATE_NONE : TRCS_OUTPUT_CURRENT_RANGE_STATE_HIGH);
errors:
    return status;
}
...
TRCS_status_t TRCS_init(void) {
    ...
    _TRCS_reset(TRCS_OUTPUT_CURRENT_RANGE_STATE_NONE);
...
TRCS_status_t TRCS_process(void) {
    ...
    if (status != TRCS_SUCCESS) goto errors;
    // Compute range.
    if (trcs_ctx.output_current_mv > TRCS_RANGE_UP_THRESHOLD_MV) {
        trcs_ctx.current_range += (!trcs_ctx.switch_pending);
    }
    if ((trcs_ctx.output_current_mv < TRCS_RANGE_DOWN_THRESHOLD_MV) && (trcs_ctx.current_range > 0)) {
        trcs_ctx.current_range -= (!trcs_ctx.switch_pending);
    }
    // Check if GPIO control is required.
    if (trcs_ctx.current_range > TRCS_OUTPUT_CURRENT_RANGE_HIGH) {
        ...
        if (trcs_ctx.current_range != trcs_ctx.previous_range) {
            // Enable new range.
            status = TRCS_HW_set_output_current_range_state(trcs_ctx.current_range, 1);
            if (status != TRCS_SUCCESS) goto errors;
            // Update state.
            trcs_ctx.range_state = (TRCS_OUTPUT_CURRENT_RANGE_STATE_LOW + trcs_ctx.current_range);
            // Start off timer.
        ...
        for (idx = 0; idx < TRCS_OUTPUT_CURRENT_RANGE_LAST; idx++) {
            ...
            if (trcs_ctx.range_ctx[idx].switch_timer_ms >= TRCS_RANGE_RECOVERY_DELAY_MS) {
                // Disable range.
                status = TRCS_HW_set_output_current_range_state(idx, 0);
                if (status != TRCS_SUCCESS) goto errors;
            }
...
TRCS_status_t TRCS_get_output_current_range_state(TRCS_output_current_range_state_t* output_current_range_state) {
    ...
    TRCS_status_t status = TRCS_SUCCESS;
    // Check parameter.
    if (output_current_range_state == NULL) {
        status = TRCS_ERROR_NULL_PARAMETER;
        goto errors;
    }
    (*output_current_range_state) = trcs_ctx.range_state;
errors:
    return status;
}
...
TRCS_status_t TRCS_get_output_current(int32_t* output_current_ua) {
    ...
    int64_t den = 0;
    // Check parameter.
    if (output_current_ua == NULL) {
        status = TRCS_ERROR_NULL_PARAMETER;
        goto errors;
    }
    // Voltage to current conversion.
    num = (((int64_t) trcs_ctx.output_current_mv) * ((int64_t) MATH_POWER_10[6]));
    den = (((int64_t) TRCS_LT6105_VOLTAGE_GAIN) * ((int64_t) TRCS_SHUNT_RESISTOR_MOHM[trcs_ctx.current_range]));
    // Compute output current.
    (*output_current_ua) = (int32_t) ((num) / (den));
errors:
    return status;
}
